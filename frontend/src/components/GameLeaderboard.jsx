          ...
          <div className="mb-6 p-4 border-2 border-cyan-400 bg-cyan-900/10 rounded-lg">
            <div className="text-center mb-4">
              <div className="text-lg font-bold text-cyan-400 uppercase tracking-wider">
                &gt; YOUR RANK &lt;
                {isUserStatsLoading && (
                  <span className="ml-2 text-xs text-cyan-400/70">UPDATING...</span>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div className="space-y-1">
                <div className="text-xs text-cyan-400/70 uppercase tracking-wider">Rank</div>
              ...
              <div className="space-y-1">
                <div className="text-xs text-cyan-400/70 uppercase tracking-wider">Score</div>
              ...
              <div className="space-y-1">
                <div className="text-xs text-cyan-400/70 uppercase tracking-wider">Progress</div>
                <div className="text-sm font-bold text-purple-400">
                  {userStats.passed_levels?.length || 0}P / {userStats.failed_levels?.length || 0}F
                </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          {currentData.length > 0 ? (
            currentData.map((item, index) => {
              const isCurrentUser = userStats && item.user_id === userStats.user_id;
              return (
                <div
                ...
                >
                  <span className={`font-bold ${isCurrentUser ? "text-cyan-400" : "text-yellow-400"}`}>
                    #{item.rank}
                  </span>
                  <span className={`text-center uppercase tracking-wider text-sm truncate ${
                    isCurrentUser ? "text-cyan-300" : "text-yellow-300"
                  }`}>
                    {item.user_id}
                    {isCurrentUser && <span className="ml-1 text-cyan-400">(YOU)</span>}
                  </span>
